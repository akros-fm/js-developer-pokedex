...
function convertPokemonToLi(pokemon) {
    return `
        <li class="pokemon ${pokemon.types.map((type) => type).join(' ')}">
            ...
            <div class="pokemon-btn" id="btn-pokedetails">Detalhes</div>
...
document.addEventListener('click', function(e){
    if(e.target.innerText == "Detalhes"){
...
loadMoreButton.addEventListener('click', () => {
    offset += limit
    const qtdRecorWithdNextPage = offset + limit

    if (qtdRecorWithdNextPage >= maxRecords) {
